...
PURE_MODULES = [
    ...
    ("ScatterCore",      "src/shared/Core/ScatterCore.luau"),
    # generated by the Sky Citadel scenario kit: scatter_core.py's placements
    ("ScatterParity",    "tests/scatter_parity.luau"),
]
...
def main():
    ...
    for folder in sorted((ROOT / "src/shared/Content").iterdir()):
        ...
        for f in sorted(folder.glob("*.luau")):
            if f.name != "init.luau":
                sources.append(transform(f.read_text(encoding="utf-8")))
        # A world too big for one file is a FOLDER whose init.luau gathers
        # its children through script:GetChildren() (Scatter/SkyCitadel).
        # Rebuilt by handing that init a fake script whose children load the
        # child files, and a require that runs them.
        for sub in sorted(p for p in folder.iterdir() if p.is_dir() and (p / "init.luau").exists()):
            kids = ",\n".join(
                '\t\t{ IsA = function() return true end, __load = function()\n%s\n\t\tend }'
                % transform(c.read_text(encoding="utf-8"))
                for c in sorted(sub.glob("*.luau")) if c.name != "init.luau"
            )
            sources.append(
                '\tlocal __kids = {\n%s\n\t}\n'
                '\tlocal script = { GetChildren = function() return __kids end }\n'
                '\tlocal require = function(k) return k.__load() end\n%s'
                % (kids, transform((sub / "init.luau").read_text(encoding="utf-8")))
            )
        for src in sources:
            entries.append(
                '\tdo\n\t\tlocal m = (function()\n%s\n\t\tend)()\n'
