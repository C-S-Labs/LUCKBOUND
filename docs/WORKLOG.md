...
### Done
- `geometry_checks.py`: every face tagged with its builder; real-mesh checks
  for detached clusters and cross-builder clipping, in `validate()`. Over 324
  pieces: 0 detached, 0 clipping (the first measurement found 91 detached
  clusters in the base kit alone and several hundred across the scenario kits).
- `settle()` snaps near-misses, lifts the base kit's anti-grav accents (halos,
  hovering crystals, orrery rings) into animated props, and drops scenario
  parts that cannot be attached; `unclip()` drops scenario builds that still
  clip. Sources fixed too: icicles, frozen falls, keel roots and vines (now
  continuous tubes), rim cutting (rail bars trimmed at a gap), palisades,
  blast walls, tower plates, banners and claw rakes (ray-cast wall faces),
  the stormhawk nest, crystal eruptions (path-cleared).
- Five raider warships (`sky_citadel_ships.py`, 2.9k–5.2k tris). Placement
  rows carry `Alt`; `ScatterCore.pick` + `PropController` draw one per run.
  `Schema.validateProps` checks alternates.
- `prop_detail.py`: bevel / subdivide / facet by material, budget by size.
- Name prefixes per library; `IMPORT_MANIFEST.md` + `import_manifest.json`
  generated by the export; `BLOCKER` anchors name their mesh.
- Re-exported: base kit (36 pieces, 72 prop meshes, placements staged) and all
  seven scenario kits (357 fixed prop meshes, 206 scatter meshes).

### Decisions made
- Deliberately floating accents are props (they animate); structure never floats
  except islets and walkable stepping plates.
- Detail budget follows size rather than a flat 2,500 minimum.

### Stopped at
Everything exported and validated; nothing imported. The next steps are the
import, then content wiring, then the Fate rework: see `STATUS.md`.
