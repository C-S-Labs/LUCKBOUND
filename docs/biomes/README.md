...
| `CloudSea` | layers of drifting cloud below the map, wrapped round the camera so they never run out. Each cloud is a cluster — a flat, lobed, shaded base with squashed lit billows and `Tufts` heaped on top — of one of three kinds (`Mix`: `Cumulus`, `Stratus`, `Wisp`), each turned its own way and tone-jittered. **Avoid flat ovals:** a bank is a row of lumpy puffs, a cumulus base is no wider than its billows, and a test refuses any piece whose footprint exceeds three times its height — from above, anything flatter reads as a fried egg. Use `Wisp` only in worlds seen from the side, never ones looked down on. **Keep clouds opaque** (`Transparency` ≈ 0): see-through clouds show every inner shape and read as bubbles; the Atmosphere's haze supplies the softness. `Material` picks the finish from `AmbienceCore.CLOUD_MATERIALS`. **`CloudCeiling` is required with it:** studs below the walk plane no cloud may rise above — set it below the kit's deepest keel so clouds never touch the map |
