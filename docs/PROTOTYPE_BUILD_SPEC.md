...
## 7.6 Amendment: enemy AI and combat — number reserved, opened 2026-09-26

**Owner-directed.** `docs/ENEMY_AI.md` is the design and the mandatory build order (its §12); this amendment claims
the number its own step 0 calls for, so a second branch can never claim §7.6 for something else. Unlike §7.1–§7.5,
**this amendment does not open anything yet** — combat, enemies, bosses and their machinery stay excluded by §7's
list above until each later step of `ENEMY_AI.md` §12 lands with its own owner sign-off.

### What was opened, and what was not

| | |
|---|---|
| ✅ **The number.** §7.6 belongs to enemy AI and combat, and nothing else | this row |
| ✅ **Module names, locked**: `CombatCore`, `PerceptionCore`, `EnemyAICore`, `DifficultyCore`, `BossCore`, `TelemetryCore` (pure cores); `EnemyService`, `BossService` (the two Systems) | `ENEMY_AI.md` §3, no longer "proposals" |
| ❌ **Remotes.** None added to §4 yet — whether a player action (an attack, a dodge) needs a new client→server remote, and what it carries, is undecided | deferred to when `ENEMY_AI.md` §12 step 1 drafts the actual contracts |
| ❌ **`GameConfig` blocks.** No tick rates, caps or jitter ranges added yet | same as above |
| ❌ **Everything else in §7's exclusion list**: combat of any kind, enemies, bosses, loot's contents | unchanged; still excluded |

### Why split the claim from the opening

`ENEMY_AI.md` §12 step 0 bundles claiming the number with fixing remotes and `GameConfig` blocks, on the
assumption they'd all be decided together. The owner chose to reserve the number and lock the module names now —
both are cheap and final — while leaving remotes and config blocks for step 1, when the actual damage/status/move
