...
| **Loot, fixtures, vault keys** | **Built 2026-09-23, waiting on re-import** | Build spec §7.5. Chests (5), the treasury vault door and the sealed gate's forcefield are server-placed fixtures; birds have flapping wings. Loot rolls per player from pools that are **empty by design** (contents deferred). The vault key drops from the boss stand-in (reaching the arena) at 20%, only in maps with the treasury (in about 1 map in 5), is shared to the party, saved (profile v3), max 1 per kind. **Nothing appears until the owner re-imports** `sky_citadel_structure.fbx` + `sky_citadel_props.fbx`. `TESTING.md` Test T. |
