...
**TESTCOUNT tests, all passing.** 65 arrived with parties and expedition instances (§7.2), and the scenario layer (§7.3) added its own: compatibility, pacing bands, anti-repetition and determinism. Headless — no Roblox required. 136 of them arrived
...
| **A Kind offered by exactly one piece is a gate AND a sameness** | **Design** | The reserved-arena-Kind rule makes the Grove precede the boss on every seed, because it is the only `WIDE` provider. At 8 pieces that was the intent (Blueprint §3.2, treants gating the approach). At 12 it is the largest single limit on variety, since the arena approach can never be anything else. Fix is content: give `WIDE` to two or three pieces that each deserve to be the last thing before a boss. The gate survives, the sameness does not. Owner's call — recorded in `biomes/VERDANT_VALLEY.md` §6. |
