...
| `Content/Chunks/SkyCitadel` | ✅ 4 pieces, **FBXs exported** — [`SKY_CITADEL.md`](SKY_CITADEL.md) |
...
| Actual meshes | ⏳ all 12 are `PLACEHOLDER`; Sky Citadel's 4 are exported as FBX |
...
reserved-Kind rule has produced gating on its own: the Spire Court is the only
`ASCENT` provider, so it precedes the Crown Spire arena on every seed (asserted
over 200). Its pieces are the first in the project generated by a script and
