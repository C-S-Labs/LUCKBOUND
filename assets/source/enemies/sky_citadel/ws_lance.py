...
# WEB + CHARGE (owner, 2026-09-25): the lance is an obtainable Legendary, so the web is REAL GEOMETRY (visible in
# every view: hands, drops, inventory, icons). Roblox Beams between the ANCHOR bones below add the crackle on top
...
_out = ["--!strict",
        ...
        _T*2 + "Width = 0.012, -- strand width, metres (scaled to the model)",
        _T*2 + "Texture = \"rbxassetid://105229129478403\", -- assets/textures/lightning_strip.png (uploaded 2026-09-25)",
        _T*2 + "TextureSpeed = 2.5,",
        _T*2 + "SlideMetres = %.3f, -- how far each blade half slides open when charged" % (SLIDE + 0.0015),
        _T*2 + '-- two anchors and their distance in metres: measures the model scale at run time',
        _T*2 + 'ScaleRef = { "WebO01", "WebO07", %.4f },' % (ANCHORS["WebO01"][0] - ANCHORS["WebO07"][0]).length]
