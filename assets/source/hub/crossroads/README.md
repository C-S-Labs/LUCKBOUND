...
## In the game (2026-09-24)

`src/server/Systems/HubV2.luau` builds the hub when `HUB_CROSSROADS_V2` is in
the place. The old authored shell and the blockout remain the fallback, and
`GameConfig.HubLayout.V2.Enabled = false` switches back.

- **Placement:** each MeshPart goes to its centre in `Content/Hub/CrossroadsV2`
  (generated by `make_layout_luau.py`; re-run it after a rebuild), with a
  half turn. Studio's importer turned the model 180°.
- **Collision:** walkable pieces are made fresh from their MeshId at
  `PreciseConvexDecomposition`, so arches and doorways stay open. The
  levitator and every animated part don't collide.
- **Motion:** animated parts get HubEffects attributes (spin, bob, flicker).
